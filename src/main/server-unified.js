...
// Тривалість таймера на повернення: 60 секунд за замовчуванням. Раніше було 3 хвилини.
...
async function initAccountsStorage() {
    ...
    try {
        ...
        // Унікальний індекс: email (потрібен для відновлення пароля). Sparse — бо поле є не в всіх.
...
// Пошук акаунта за email (Atlas або локальний JSON)
    ...
    socket.on('auth-login', async ({ login, password }) => {
        ...
        if (accountsCollection) {
            try {
                ...
                socket.emit('auth-login-result', { success: true, playerId: account.id, nick: account.nick, login: account.login, cups: account.cups || 0, email: account.email || '' });
            } catch (e) {
